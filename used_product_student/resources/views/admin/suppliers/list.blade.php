...
@section('nav')
    <div class="x-nav">
            <span class="layui-breadcrumb">
              <a><cite>홈페이지</cite></a>
              <a><cite>중고품 관리</cite></a>
              <a><cite>판매자 리스트</cite></a>
            </span>
...
@section('content')
    <div class="x-body">
        ...
        <xblock>
            @if ($isCreate)
                @component('components.action_add', ['route'=>'suppliers.add'])
                @endcomponent
            @endif
            ...
            @foreach($list as $data)
                <tr>
                    ...
                    <td class="td-manage">
                        ...
                        @if ($isEdit)
                            @component('components.action_edit', ['route'=>'suppliers.edit', 'id'=>$data->id])
                            @endcomponent
                        @endif

                        @if ($data['isDelete'])
                            @component('components.action_delete', ['route'=>'suppliers.delete', 'id'=>$data->id])
                            @endcomponent
                        @endif
                    </td>
                </tr>
