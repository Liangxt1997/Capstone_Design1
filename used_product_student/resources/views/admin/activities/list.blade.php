...
@section('nav')
    <div class="x-nav">
            <span class="layui-breadcrumb">
              <a><cite>홈페이지</cite></a>
              <a><cite>{{trans('admin.title01')}}</cite></a>
              <a><cite>{{trans('admin.zixun')}}</cite></a>
            </span>
...
@section('content')
    <div class="x-body">
        ...
        <xblock>
            @if ($isCreate)
                @component('components.action_add', ['route'=>'activities.add'])
                @endcomponent
            @endif
            ...
            @foreach($list as $data)
                <tr>
                    ...
                    <td class="td-manage">
                        ...
                        @if ($isEdit)
                            @component('components.action_edit', ['route'=>'activities.edit', 'id'=>$data->id])
                            @endcomponent
                        @endif

                        @if ($data['isDelete'])
                            @component('components.action_delete', ['route'=>'activities.delete', 'id'=>$data->id])
                            @endcomponent
                        @endif
                    </td>
                </tr>
