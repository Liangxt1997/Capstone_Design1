...
@section('content')
    <div class="x-body">
        ...
        <xblock>
            @if ($isCreate)
                @component('components.action_add', ['route'=>'downloads.add'])
                @endcomponent
            @endif
                ...
                <tr>
                    ...
                    <td class="td-manage">
                        ...
                        @if ($isEdit && $data->status > 1 && $data->status != 98 && $data->status != 99)
                            @component('components.action_edit', ['route'=>'downloads.edit', 'id'=>$data->id])
                            @endcomponent
                            状态
                        @endif
...
{{--                            @endif--}}
                        ...
                        @if ($data['isDelete'])
                            @component('components.action_delete', ['route'=>'downloads.delete', 'id'=>$data->id])
                            @endcomponent
                        @endif
                    </td>
                </tr>
